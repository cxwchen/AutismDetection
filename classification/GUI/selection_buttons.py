...
def subject_sex_btn(subjects_frame, context, Xtrain, ytrain):
    print(Xtrain.size)
...
def subject_age_btn(subjects_frame, context, Xtrain, ytrain):
    print(Xtrain.size)
    # Define shared variable at module level if needed
    subject_functions = {
        "All": "All",
...
def select_btn(features_frame, context, Xtrain, ytrain):
    print(Xtrain.size)
    # Define shared variable at module level if needed
    featselect_functions = {
        "cluster": applySVM,
        "mRMR": applyLogR,
        "lasso": applyRandForest,
        "hsiclasso": applyDT
    }

    featselect_var = tk.StringVar(value="cluster")

    def on_select():
        selected = featselect_var.get()
        context.features_set = selected
        context.model = featselect_functions[selected](Xtrain, ytrain)
        try:
            HR_V1_0_03.update_overview_text(context)
        except Exception as e:
            print(f"[Visualizer] Could not update overlay text: {e}")
